...
class WP_CLI_CONFIG {
	...
	public function get_global_config_path() {
		...
		if ( empty( $path ) ) {
			$path = WP_CLI_FileSystem::path_join( WP_CLI_Helper::get_home_path(), $this->config_filename );
		}
	...
	public function get_current_directory_config_file() {
		...
		foreach ( $this->config_working_file as $file ) {
			$path = WP_CLI_FileSystem::path_join( $cwd, $file );
			if ( file_exists( $path ) ) {
				$file_path = $path;
			}
		}
		...
		if ( isset( $file_path ) ) {
			...
			return WP_CLI_FileSystem::path_join( $cwd, $this->config_working_file[1] );
		}
	}
	...
	public static function get( $arg = false, $where = false ) {
		...
		if ( ! $arg ) {
			...
			if ( is_string( $arg ) ) {
				...
				$exist_key = WP_CLI_Util::check_exist_key( $arg, $list );
				if ( $exist_key != false ) {
					$exp = $list;
					foreach ( $arg as $key ) {
						$exp = $exp[ $key ];
	...
	public function remove_config_file() {
		WP_CLI_FileSystem::remove_file( $this->config_path );
	}
}