...
class WP_CLI_Helper
{
    ...
    public static function get_home_path($path = '')
    {
        ...
        return WP_CLI_FileSystem::path_join($home_path, $path);
    }
    ...
    public static function get_cache_dir($path = '')
    {
        if (getenv('WP_CLI_CACHE_DIR')) {
            ...
            $cache = WP_CLI_FileSystem::path_join(self::get_home_path(), "cache/");
        }

        return WP_CLI_FileSystem::path_join($cache, $path);
    }
    ...
    public static function exist_cache_file($file_path)
    {
        ...
        if (file_exists($path)) {
            return WP_CLI_FileSystem::normalize_path($path);
        }
    ...
    public static function run_command($command, $option = array())
    {
        ...
        if ( ! empty($option)) {
            $options = WP_CLI_Util::parse_args($option, $options);
        }
    ...
    public static function run_composer($path, $args)
    {
        ...
        $composer_path = WP_CLI_FileSystem::normalize_path($path);
    ...
    public static function exist_wp_cli_command($cmd)
    {
        ...
        $cmd = trim(\WP_CLI_Util::remove_whitespace_word($cmd));

        //Remove Default Prefix
        $ext     = explode(" ", $cmd);
        $command = array();
        if (in_array(strtolower($ext[0]), array('wp-cli.phar', 'wp', 'wordpress', 'wpcli.phar', 'wp-cli', 'wpcli'))) {
    ...
    static public function command_exists($command)
    {
        ...
        $command = WP_CLI_Util::remove_whitespace_word(trim($command[0]));
    ...
    public static function lunch_editor($file_path, $editor = false)
    {
        ...
        if (self::is_windows() and $editor != false) {
            ...
            if ( ! in_array($editor, array_keys($win_editor))) {
                WP_CLI_Helper::error("Editor's name is not valid");
            } else {
                # Check installed Current Editor
                if (self::command_exists($win_editor[$editor]) === false) {
                    self::error("'" . self::color($win_editor[$editor], "Y") . "' is not recognized as an internal or external command.");
                } else {
            ...
            # Lunch with WP-CLI
            $r = \WP_CLI\Utils\launch_editor_for_input($contents, basename($file_path), pathinfo($file_path, PATHINFO_EXTENSION));
            if ($r === false) {
                WP_CLI_Helper::warning('No changes made to ' . basename($file_path));
            } else {
